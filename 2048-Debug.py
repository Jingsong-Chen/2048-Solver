...
class Game:
	...
	def loop(self, fromLoaded = False):
		...
		if not fromLoaded:
			...
			self.placeRandomTile()
		# display the matrix on GUI
		self.printMatrix()
		while True:
			# if the score is higher than 5000, end the game
			# if self.total_points >= 5000:
			# 	self.endGame()
			# 	if self.GAMES_DONE == self.TOTAL_GAME:
			# 		break
			# if in auto mode
			if auto:
				# check if the game can continue
				if self.checkIfCanGo() and self.doesNotHave2048():
					#print("AI's thinking:")
					# make an actual copy rather than make a reference
					ai = Gametree(copy.deepcopy(self.tileMatrix), 3, self.total_points)
					direction = ai.compute_decision()
					#direction = ai.trivial_decision()
					#print("choose")
					#print(direction)
					self.move(direction)
				# game over
					...
					self.GAMES_DONE += 1
					if not self.doesNotHave2048():
						self.GOOD_GAME += 1
						print\
							("game", self.GAMES_DONE, "good game", "current ratio:", self.GOOD_GAME / self.GAMES_DONE, "score:", self.total_points)
						self.reset()
					else:
						print\
							("game", self.GAMES_DONE, "bad game", "current ratio:", self.GOOD_GAME / self.GAMES_DONE, "score:", self.total_points)
						auto = False
					if self.GAMES_DONE == self.TOTAL_GAME:
						auto = False
						print("Good Game:")
						print(self.GOOD_GAME)
						print("Total Game:")
						print(self.GAMES_DONE)
						print("Good Game rate:")
						print(self.GOOD_GAME / self.GAMES_DONE)
						self.printGameOver()
			...
			for event in pygame.event.get():
				...
				if event.type == QUIT:
					pygame.quit()
					sys.exit()
				# of enter key is pressed, toggle the auto mode
				if event.type == KEYDOWN and event.key == K_RETURN:
					auto = not auto
				# if an arrow key is pressed and the game is not over, make a move in that direction
				...
				# game over
					...
					self.printGameOver()
				# r for reset, s for save, l for load, u for undo
				if event.type == KEYDOWN:
	...
	def move(self, direction):
		# before we move, take a snapshot so that we can undo if we want to
		self.addToUndo()
		# rotate the matrix to the up direction
	...
	def doesNotHave2048(self):
		for i in range(0, 4):
			for j in range(0, 4):
				if self.tileMatrix[i][j] == 2048:
					return False
		return True
	...
	def placeRandomTile(self):
		while True:
			...
			if self.tileMatrix[i][j] == 0:
				break
		# the value of the new tile is always 2
